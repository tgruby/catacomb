...
export default class EnemyVitals extends Grid {
  constructor () {
    ...
    memory.subscribe({ key: 'enemy.health', callback: this.updateHealth.bind(this) })
  }
  ...
  updateHealth (health) {
    if (health.current <= 0 && health.max > 0) {
      ...
      this.add({ x: 2, y: 3, string: 'Health' })
      this.add({ x: 4, y: 5, string: '♡'.repeat(heartCount) + ' '.repeat(blankCount) })
    }
  }
}
