...
export default class Catacombs {
  ...
  constructor () {
    ...
    memory.subscribe({ key: 'catacombs.next.level', callback: this.startNewLevel.bind(this) })
  }
  ...
  addExit () {
    ...
    const lowerRight = locationHash(this.yOffset(this.height - 1), this.xOffset(this.width - 1))
    if (this.level % 2 === 1) {
      const up = (this.level === 1) ? loader.get('feature.entrance') : loader.get('feature.up')
  ...
  replaceSpacesWithDoors (xPos, yPos) {
    ...
    if (wallDirections.length == 3) {
      if (wallDirections.includes('north') && !wallDirections.includes('south')) {
        ...
        this.entities[locationHash(yPos, xPos)] = loader.get('container.sarcophagus')
