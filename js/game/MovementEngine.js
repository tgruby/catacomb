...
export default class MovementEngine {
  constructor (hero) {
    this.hero = hero
    memory.subscribe({ key: 'hero.position', callback: this.setPointOfView.bind(this) })
    this.setPointOfView()
  }
  ...
  moveForward () {
    ...
    if (current !== undefined) {
      if (current.type === 'creature' && current.allowsDisengage === false) {
        memory.set({ key: 'message.center', value: `${current.name} blocks your path!` })
        return
      }
    }
    ...
    if (stamina.value < 1) {
      ...
      memory.set({ key: 'message.center', value: `${hero} is too tired to move!` })
      return
    }
    ...
    if (next !== undefined && next['obstructs.movement']) {
      ...
      memory.set({ key: 'message.center', value: `${obstruction} blocks your path!` })
      return
    }
  ...
  setPointOfView () {
    ...
    const nearEntity = this.getFeatureAt({ y: position.y + near[1], x: position.x + near[2] })
    const midEntity = this.getFeatureAt({ y: position.y + middle[1], x: position.x + middle[2] })
    const farEntity = this.getFeatureAt({ y: position.y + far[1], x: position.x + far[2] })
    ...
    if (response.hereEntity) {
      ...
      memory.set({ key: 'message.center', value: `You see ${nameArticle} ${name}` })
    }
  ...
  getFeatureAt (props) {
    ...
    if (symbol === '|' || symbol === '+' || symbol === '-') return loader.get('feature.wall')
  }
