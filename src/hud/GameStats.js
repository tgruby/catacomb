...
export default class GameStats extends Grid {
  constructor () {
    ...
    this.add({ x: 'center', y: 0, string: ' ' + memory.get('hero.first.name') + ' ' + memory.get('hero.last.name') + ' ', force: true })
    ...
    memory.subscribe({ key: 'hero.level', callback: this.updateHeroLevel.bind(this) })
    memory.subscribe({ key: 'catacombs.level', callback: this.updateCatacombsLevel.bind(this) })
    memory.subscribe({ key: 'hero.xp', callback: this.updateXP.bind(this) })
  }

  updateHeroLevel (level) {
    this.add({ x: 'left', y: 2, string: `Level ${level} (${getHeroLevelTitle(level)})` })
