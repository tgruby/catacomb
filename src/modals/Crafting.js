...
export default class Crafting extends Modal {
  constructor (props) {
    super({ id: 'CraftingModal', width: 64, height: 35, border: true, parent: props.parent })

    this.add({ x: 'center', y: 2, string: ' Crafting Menu' })

    const items = this.getInventorySummary()
    const craftingSkills = new SelectionArray({ id: 'CraftingSkills', width: 32, height: 30, border: true, items })
    inventoryItems.add({ x: 'left', y: 0, string: ' Skills ', force: true })
